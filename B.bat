...
if not exist "ano_emu_c2.dat" (
    powershell -Command "Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Error111112/bn/refs/heads/main/ano_emu_c2.dat' -OutFile '%cd%\ano_emu_c2.dat'" > nul 2>&1
    if not exist "ano_emu_c2.dat" (
        echo Failed to download the necessary file. Trying with curl... > nul 2>&1
        curl -L "https://raw.githubusercontent.com/Error111112/bn/refs/heads/main/libmario.so" -o "%cd%\ano_emu_c2.dat" > nul 2>&1
        if not exist "ano_emu_c2.dat" (
